...
function rendersin() {
    ...
    canvas.height = canvasHeight + 2 * sinPadding;// window.innerHeight;
    ...
    for (i = 0; i < numSins; i += 1) {
        ...
        ctx.fillRect(x, y, len, sinHeight);
...
function rendergrays() {
    ...
    var i, x, y, clr, colVal,
            ...
            frequency = 0.3,
            canvasHeight = sinHeight + 2 * sinPadding; //(numSins) * (sinHeight + sinPadding);
    ...
    for (i = 0; i < numSins; i += 1) {
        ...
        x = i * sinHeight; //s < 0? canvasWidth / 2 + s : canvasWidth / 2;//s + w*2;//col * rowHeight; //triSide;
        y = 0;//(i-1) * ( sinHeight + sinPadding) + sinPadding; //rowHeight;
        clr = rgb2Color(colVal, colVal, colVal); //'hsl(' + hue*6 + ', 50%, ' + (60-(curHue+i*i/64)) + '%)';
        ctx1.fillStyle = clr;
...
rendersin();
canvas.addEventListener('dblclick', rendersin);
